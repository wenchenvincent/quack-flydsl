...
"""128×128 tile GEMM with L2-friendly tile rasterisation — EXPERIMENTAL,
marginal win at specific shapes.

At 4096² the ``gemm_gfx950_128x128.py`` kernel is HBM-bandwidth bound.
Hypothesis: grouping tiles so that consecutive workgroups share A-rows
would reduce redundant fetches by keeping A hot in L2. This kernel
launches ``grid=(num_total_tiles, 1, 1)`` and remaps the linear block
id to (bid_m, bid_n) using a ``group_size``-N rasterisation so
workgroups 0..group_size-1 all share the same A-row.

**MI355X f16 measured result (time per call, smaller = better):**

    shape                base     swz      base/swz
    1024²                36.9     36.8     1.001x (flat)
    2048²                104.3    102.3    1.02x  (marginal)
    2048×2048×K=4096     215.0    203.1    1.06x  (real but small)
    4096²                322.2    332.6    0.97x  (regression)

**Finding**: MI350's 32 MB L2 is large enough that the natural
(block_idx.x, block_idx.y) grid order already gets most of the
cache benefit. Explicit swizzle only shows up at tall-and-thin K
where the working set just fits in L2 with help.

The 4096² regression is likely from the extra modular-arithmetic
overhead per WG and slight cold-cache behaviour when group tail
blocks misalign.

Keeping the file as a reference for future persistent/stream-K
work where explicit tile ordering interacts with CU-local state.
...
def _build_gemm_128x128_swz(*, M, N, K, dtype_str, arch, group_size):
    ...
    allocator = SmemAllocator(
        None, arch=arch,
        global_sym_name=f"quack_amd_gemm_128x128_swz_{dtype_str}_{M}_{N}_{K}_g{group_size}_smem",
    )
    ...
    def kernel(A: fx.Tensor, B: fx.Tensor, C: fx.Tensor):
        lin_bid = fx.block_idx.x
        # Group-of-N rasterisation. Handles non-divisible tail by
        # clamping the last group to the remaining n-tiles.
        grp_tiles = fx.Int32(group_size * tiles_m)
        grp_id = lin_bid // grp_tiles
        in_grp = lin_bid % grp_tiles
        bid_m = in_grp // fx.Int32(group_size)
        bid_n = grp_id * fx.Int32(group_size) + (in_grp % fx.Int32(group_size))
    ...
    def launch(A: fx.Tensor, B: fx.Tensor, C: fx.Tensor,
        ...
        with ir.InsertionPoint(ctx.gpu_module_body):
            allocator.finalize()
        kernel(A, B, C).launch(
            grid=(total_tiles, 1, 1), block=(256, 1, 1), stream=stream,
        )
...
@torch.library.custom_op(
    "quack_amd::_gemm_128x128_swz_out",
    mutates_args=("out",),
    schema="(Tensor a, Tensor b, Tensor(a0!) out) -> ()",
)
def _gemm_128x128_swz_out(a: Tensor, b: Tensor, out: Tensor) -> None:
    ...
    tiles_n = N // 128
    group_size = min(_GROUP_SIZE, tiles_n)
    _compile(M, N, K, _DTYPE2STR[a.dtype], get_rocm_arch(), group_size)(a, b, out)


@_gemm_128x128_swz_out.register_fake
def _gemm_128x128_swz_out_fake(a, b, out):
    return None


def gemm_128x128_swz(A: Tensor, B: Tensor) -> Tensor:
    """128×128-tile GEMM with group-of-N rasterisation for L2 reuse.
    ...
    _gemm_128x128_swz_out(A, B, out)
    return out


__all__ = ["gemm_128x128_swz"]
