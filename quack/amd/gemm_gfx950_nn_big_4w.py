...
"""NN-layout GEMM — 4-wave interleave variant (HipKittens-style).

Companion to ``gemm_gfx950_nn_big.py``. The big kernel uses 8 waves
(2 per SIMD) at a 256×256 output tile; this variant uses 4 waves
(1 per SIMD) at a smaller 128×256 output tile.

Why 4-wave interleave (per HipKittens, arxiv:2511.08083):
With 1 wave per SIMD and a smaller WG (4 waves × 64 = 256 threads),
2 workgroups can co-reside on a single CU (MI355X has 64 KB VGPR/SIMD
budget per wave). The CU keeps 8 waves total — same count as the
8-wave variant — but split across 2 independent WGs working on
different output tiles. When one WG's wave stalls on memory or LDS,
the other WG's wave on the same SIMD can issue.

The per-wave instruction stream is the standard "compute + memory
carefully staggered" interleave (our ``hot_loop_scheduler`` hints
already produce this). The win comes from doubled CU-level WG
occupancy, not from changing the per-wave code shape.

LDS budget (per WG) at 128×128 tile:
    A: STAGES=2 × 128 × 64 × 2B  = 32 KB
    B: STAGES=2 × 64 × 136 × 2B  = 34 KB  (136 = 128 + 8 pad)
    C staging: 128 × 64 × 2B     = 16 KB  (half of 128×128, 2-pass)
    Total = max(A_single=16K, C=16K) + B=34K = 50 KB per WG
    Two WGs per CU consume 100 KB combined; well under the 160 KB cap.
    This is the key to 4-wave winning: 2 WGs (= 8 waves total, same as
    the 8-wave variant) co-reside on each CU and the SIMD scheduler
    issues across WGs when one stalls.

Workgroup size: 4 warps × 64 lanes = 256 threads/WG.
(no ``known_block_size`` override needed — 256 ≤ default cap.)

Public API:

    gemm_nn_big_4w(a, b, out=None)
        a: (M, K) f16/bf16, K stride-1 (inner)
        b: (K, N) f16/bf16, N stride-1 (inner)
        out: optional preallocated (M, N) in same dtype

    Constraints: M % 128 == 0 and N % 256 == 0; K % 64 == 0.
"""
...
def _compile_nn_big_4w_kernel(
    ...
    allocator = SmemAllocator(
        None, arch=GPU_ARCH,
        global_sym_name=f"nn_big_4w_smem_{dtype}_{k}_{n}_{BLOCK_M}_{BLOCK_N}",
    )
    ...
    KERNEL_NAME = f"nn_big_4w_{dtype}_{BLOCK_M}x{BLOCK_N}x{BLOCK_K}_S{STAGES}"
...
def gemm_nn_big_4w(a: Tensor, b: Tensor, out: Optional[Tensor] = None) -> Tensor:
    """128×256×64 / 4-warp NN-layout GEMM (HipKittens-style interleave).

    Companion to ``gemm_nn_big`` (8-wave). With 4 waves per WG (256
    threads), 2 WGs co-reside per CU on MI355X — same total CU
    wave count as the 8-wave variant, but split across 2 independent
    output tiles, improving latency hiding when one WG's wave stalls.

    Constraints: M % 128 == 0 and N % 128 == 0; K % 64 == 0.
    """
    ...
    assert M % 128 == 0 and N % 128 == 0 and K % 64 == 0, (
        f"gemm_nn_big_4w requires M%128 and N%128 and K%64; got {M}×{K}×{N}"
    )
    ...
    _compile_nn_big_4w_kernel(dtype_str, K, N, _m_hint=M)(out, a, b, M)
    return out


__all__ = ["gemm_nn_big_4w"]
