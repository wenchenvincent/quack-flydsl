...
Public API:
    ...
    gemm_nt_4wave_hkp(a, b, out=None)
...
def _compile_nt_4wave_hkp_kernel(
    ...
    allocator = SmemAllocator(
        None, arch=GPU_ARCH,
        global_sym_name=f"nt_4wave_hkp_smem_{dtype}_{k}_{n}",
    )
    ...
    KERNEL_NAME = f"nt_4wave_hkp_{dtype}_{BLOCK_M}x{BLOCK_N}x{BLOCK_K}"

    @flyc.kernel(known_block_size=[BLOCK_THREADS, 1, 1])
    def nt_kernel_4wave_hkp(C: fx.Tensor, A: fx.Tensor, B: fx.Tensor,
                            timing: fx.Tensor, m: fx.Int32):
        # timing: i64[NUM_WGS, 5] — per-WG cluster-boundary timestamps
        # captured during the LAST main-loop iter (steady state).
        ...
        A0_LEN = M_ATOMS_PER_QUAD * K_SUBITERS
        B0_LEN = N_ATOMS_PER_QUAD * K_SUBITERS

        ts_init = arith.constant(0, type=T.i64)
        init_state = ([k_zero, arith.constant(0, index=True)]
                      + c_frags + a0_init + b0_init
                      + [ts_init, ts_init, ts_init, ts_init, ts_init])
        ...
        for _ki, state in range(0, TOTAL_K_STEPS - 2, init=init_state):
            ...
            # === c0 ===
            # Drain to vmcnt(4) — keep some prefetches in flight, but ensure
            # the LDS reads we're about to do are safe (their data must be
            # committed from the prefetch 2 K-steps ago).
            rocdl.s_waitcnt(VMCNT_4)
            rocdl.s_waitcnt(LGKMCNT_0)
            gpu.barrier()

            # Load b[1] from Bs[curr][1] = K's B_h1
            b1_frags = lds_matrix_b_half(stage_curr, 1)
            # Prefetch As[curr][0] for K+2 (4 chunks for A side)
            ...
            # === c1 ===
            gpu.barrier()
            # Load a[1] from As[curr][1] = K's A_h1
            a1_frags = lds_matrix_a_half(stage_curr, 1)
            # Prefetch Bs[curr][0] for K+2 (3 chunks for B side)
            ...
            # === c2 ===
            # Drain vmcnt for the next iter's first-operand loads.
            rocdl.s_waitcnt(VMCNT_4)
            gpu.barrier()
            # Load a[0]_new from As[next][0] = K+1's A_h0
            a0_next_frags = lds_matrix_a_half(stage_next, 0)
            # Prefetch Bs[curr][1] for K+2 (3 more chunks)
            ...
            # === c3 ===
            gpu.barrier()
            # Load b[0]_new from Bs[next][0] = K+1's B_h0
            b0_next_frags = lds_matrix_b_half(stage_next, 0)
            # Prefetch As[curr][1] for K+2 (4 more chunks)
        ...
        # ----- Extract timing from final loop state (last 5 entries are ts0..ts4) -----
        TS_OFF = 2 + C_FRAGS_LEN + A0_LEN + B0_LEN
        ts0_final = results[TS_OFF + 0]
        ts1_final = results[TS_OFF + 1]
        ts2_final = results[TS_OFF + 2]
        ts3_final = results[TS_OFF + 3]
        ts4_final = results[TS_OFF + 4]
        ...
        # ----- Thread 0 of WG writes the 5 timestamps to timing[wg, :] -----
        from flydsl._mlir.dialects import scf as _scf_p
        T_ = GTensor(timing, dtype=T.i64, shape=(-1, 5))
        is_thread0 = arith.cmpi(
            arith.CmpIPredicate.eq, tid, fx.Int32(0),
        )
        if_op_p = _scf_p.IfOp(is_thread0, results_=[], has_else=False)
        with ir.InsertionPoint(if_op_p.then_block):
            wg_idx = fx.Index(flat_pid)
            T_.vec_store((wg_idx, fx.Index(0)), ts0_final, 1)
            T_.vec_store((wg_idx, fx.Index(1)), ts1_final, 1)
            T_.vec_store((wg_idx, fx.Index(2)), ts2_final, 1)
            T_.vec_store((wg_idx, fx.Index(3)), ts3_final, 1)
            T_.vec_store((wg_idx, fx.Index(4)), ts4_final, 1)
            _scf_p.YieldOp([])
    ...
    def launch_nt_kernel(
        ...
        bn = n // BLOCK_N
        total_tiles = bm * bn
        nt_kernel_4wave_hkp._func.__name__ = KERNEL_NAME
        launcher = nt_kernel_4wave_hkp(C, A, B, timing, m)
...
def gemm_nt_4wave_hkp(
    ...
    assert M % 256 == 0 and N % 192 == 0 and K % 128 == 0, (
        f"gemm_nt_4wave_hkp requires M%256, N%192, K%128; got {M}×{K}×{N}"
    )
    ...
    num_wgs = (M // 256) * (N // 192)
    timing = torch.zeros(num_wgs, 5, dtype=torch.int64, device=a.device)
    dtype_str = _DTYPE2STR[a.dtype]
    _compile_nt_4wave_hkp_kernel(dtype_str, K, N, _m_hint=M)(out, a, b, timing, M)
    return out, timing


__all__ = ["gemm_nt_4wave_hkp"]
