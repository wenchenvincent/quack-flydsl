...
def _compile_nt_4wave_kernel(
    ...
    def nt_kernel_4wave(C: fx.Tensor, A: fx.Tensor, B: fx.Tensor,
                        timing: fx.Tensor, m: fx.Int32):
        # timing: i64[NUM_WGS, 5] — per-WG cluster boundary timestamps
        #   timing[wg, 0] = before c0
        #   timing[wg, 1] = after c0 (before c1)
        #   timing[wg, 2] = after c1 (before c2)
        #   timing[wg, 3] = after c2 (before c3)
        #   timing[wg, 4] = after c3
        ...
        # ----- s_memrealtime helper (returns i64) -----
        def now():
            return llvm.call_intrinsic(
                T.i64, "llvm.amdgcn.s.memrealtime", [], [], [],
            )

        # 5 timestamps per iter; overwritten each iter so only the LAST
        # iter's values survive in scf.for state.
        ts0_init = arith.constant(0, type=T.i64)
        BLOCK_K_LOOPS = k // BLOCK_K
        init_state = ([k_begin, arith.constant(0, index=True)]
                      + c_frags
                      + [ts0_init, ts0_init, ts0_init, ts0_init, ts0_init])
        for _bki, state in range(1, BLOCK_K_LOOPS, init=init_state):
            ...
            # ----- Capture cluster boundary timestamps -----
            ts0 = now()

            # ----- Rebalanced cluster scheduling (a_h1 loaded upfront) -----
            ...
            # c1: bare mma, no side-task ds_reads.
            mma_quadrant(a_h0, b_h1_frags, c_frags, 1)
            rocdl.sched_barrier(0)
            ts2 = now()
            rocdl.sched_barrier(0)

            # c2: mma C[1][0] interleaved with B HBM prefetch.
            c2_side_tasks = []
            for i in range_constexpr(LDG_B_REG_COUNT):
        ...
        # ----- Extract timing from final loop state -----
        # Last 5 entries are ts0..ts4 from the last main loop iter.
        ts0_final = results[2 + C_FRAGS_LEN + 0]
        ts1_final = results[2 + C_FRAGS_LEN + 1]
        ts2_final = results[2 + C_FRAGS_LEN + 2]
        ts3_final = results[2 + C_FRAGS_LEN + 3]
        ts4_final = results[2 + C_FRAGS_LEN + 4]
        ...
        if_op = scf.IfOp(is_thread0, results_=[], has_else=False)
    ...
    def launch_nt_kernel(
        ...
        launcher = nt_kernel_4wave(C, A, B, timing, m)
...
def gemm_nt_4wave_profile(
    a: Tensor, b: Tensor, out: Optional[Tensor] = None,
):
    """4-wave NT GEMM with per-cluster timing instrumentation.

    Returns (out, timing) where ``timing`` is an i64 tensor of shape
    ``(NUM_WGS, 5)`` containing per-WG cluster boundary timestamps from
    the LAST main-loop iter:

        timing[:, 0] = before c0
        timing[:, 1] = after c0 (= before c1)
        timing[:, 2] = after c1
        timing[:, 3] = after c2
        timing[:, 4] = after c3

    Per-cluster latency in cycles = timing[:, i+1] - timing[:, i].
    ...
    _compile_nt_4wave_kernel(dtype_str, K, N, _m_hint=M)(out, a, b, timing, M)
    return out, timing


__all__ = ["gemm_nt_4wave_profile"]
