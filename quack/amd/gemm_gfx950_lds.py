...
def _build_gemm_32x32_lds(*, M, N, K, dtype_str, arch):
    ...
    def kernel(A: fx.Tensor, B: fx.Tensor, C: fx.Tensor):
        ...
        def _stage_tile_into_lds(k_tile_scalar, stage):
            ...
            A loads use BufferCopy64b (4 contiguous K elements per
            fragment → 1 HBM instruction per fragment). B stays scalar
            because B's 4 K elements come from different rows.
            """
            ...
            # A top+bot halves: one vec4 HBM load per 4 LDS stores.
