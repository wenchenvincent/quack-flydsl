...
"""128×128 tile MFMA GEMM — B-transposed LDS — EXPERIMENTAL.

Variant of ``gemm_gfx950_128x128.py`` with B stored in LDS as
``(N, K+pad)`` instead of ``(K, N)`` row-major, with K-stride padding
to avoid bank conflicts on reads.

**Rationale (theoretical)**: per-MFMA B-fragment becomes a 4 K-contig
elements read (vec4 shape) instead of 4 scalar reads across
non-contiguous K rows. Expected ~8× fewer LDS cycles on B reads.

**MI355X measured results (f16):**

    shape       128×128    128×128_bt    bt/128
    128²        17 μs      13 μs         1.38×   small-shape win
    256²        16 μs      16 μs         1.03×   marginal
    512²        29 μs      29 μs         1.02×   marginal
    1024²       59 μs      57 μs         1.04×   marginal
    2048²       114 μs     110 μs        1.04×   marginal
    4096²       369 μs     396 μs        0.93×   regression

The LDS B-read path isn't the dominant bottleneck at shapes where we
have enough compute to hide LDS latency (2048²+). At 4096² the extra
LDS footprint (+2 f16 padding per N row = 512 bytes more per stage)
may reduce per-CU occupancy slightly. The 128² win is real but that
range is already served by ``4wave_64x64`` (the dispatcher never
picks 128×128 for 128-byte shapes).

Kept in-tree as a reference implementation and an anchor if future
MFMA issue-rate improvements shift the balance. **Not wired into the
autotune dispatcher.**
"""
...
_WAVE_ROWS = 4
_WAVE_COLS = 4
_B_K_PAD = 2   # f16 elements of padding per B_T row to break bank conflicts

_A_TILE_BYTES = 128 * 16 * 2
_B_TILE_BYTES = 128 * (16 + _B_K_PAD) * 2   # (N, K+pad) layout
...
def _build_gemm_128x128_bt(*, M, N, K, dtype_str, arch):
    assert M % 128 == 0 and N % 128 == 0 and K % _MFMA_K == 0
    assert dtype_str in ("f16", "bf16")

    allocator = SmemAllocator(
        None, arch=arch,
        global_sym_name=f"quack_amd_gemm_128x128_bt_{dtype_str}_{M}_{N}_{K}_smem",
    )
    ...
    K_STRIDE = 16 + _B_K_PAD   # f16 elements per B_T row
    ...
    def kernel(A: fx.Tensor, B: fx.Tensor, C: fx.Tensor):
        ...
        # B stored transposed with padding: (N=128, K_STRIDE).
        s_B_T = [
            SmemPtr(base_ptr, b_off0, in_elem_type, shape=(128, K_STRIDE)),
            SmemPtr(base_ptr, b_off1, in_elem_type, shape=(128, K_STRIDE)),
        ]
        for sp in (*s_A, *s_B_T):
            sp.get()
        ...
        def _stage_load(k_tile_scalar, stage):
            ...
            # --- A: unchanged (K, N)-row-major layout ---
            ...
            # --- B: HBM still (K, N); LDS transposed to (N, K+pad) ---
            ...
            # 8 scalar writes, each to LDS_T[n_col, k_row] — 8 different rows,
            # same column → 8 distinct banks per cycle.
            for i in range_constexpr(8):
                s_B_T[stage].store(
                    b_chunk[i].ir_value(),
        ...
        def _consume(stage, accs):
            # A fragments unchanged (vec4 from row-major (M, K) LDS).
            a_frags = []
            ...
            # B fragments from transposed LDS — 4 K-contig elements per frag.
            # Still scalar loads because we don't have a vectorized LDS load
            # primitive tied to SmemPtr; the hope is the compiler / hardware
            # schedules these as a single cycle (contiguous addresses in one
            # bank pattern).
            b_frags = []
            for c in range_constexpr(_WAVE_COLS):
                ...
                for i in range_constexpr(_FRAG_B):
                    ...
                    b_vals.append(s_B_T[stage].load([_idx(b_col_l), _idx(k_in_lds)]))
...
def _compile(M, N, K, dtype_str, arch):
    ...
    if fx.const_expr(got is None):
        got = _build_gemm_128x128_bt(M=M, N=N, K=K, dtype_str=dtype_str, arch=arch)
        _kernel_cache[key] = got
    return got


@torch.library.custom_op(
    "quack_amd::_gemm_128x128_bt_out",
    mutates_args=("out",),
    schema="(Tensor a, Tensor b, Tensor(a0!) out) -> ()",
)
def _gemm_128x128_bt_out(a: Tensor, b: Tensor, out: Tensor) -> None:
...
@_gemm_128x128_bt_out.register_fake
def _gemm_128x128_bt_out_fake(a, b, out):
    return None


def gemm_128x128_bt(A: Tensor, B: Tensor) -> Tensor:
    """128×128 MFMA GEMM with B-transposed LDS layout (bank-conflict-
    breaking padding on the K-stride)."""
    assert A.is_cuda and B.is_cuda
    assert A.dtype == B.dtype and A.dtype in (torch.float16, torch.bfloat16)
    M, K = A.shape
    _, N = B.shape
    out = torch.empty(M, N, device=A.device, dtype=torch.float32)
    _gemm_128x128_bt_out(A, B, out)
    return out


__all__ = ["gemm_128x128_bt"]
